...
public static class TimerHelper
{
    ...
    /// <summary>
    /// Функция вызова асинхронного экспоненциального таймера
    /// </summary>
    /// <param name="iteration">Номер итерации в цикле</param>
    /// <param name="startValue">Стартовое значение таймера (рекомендуется 6.5)</param>
    /// <param name="step">Шаг изменения значения таймера между итерациями (рекомендуется 0.1)</param>
    public static async Task ExpWaitAsync(int iteration, double startValue = 6.5, double step = 0.1) =>
        await Task.Delay((int)Math.Exp(startValue - step * iteration));
}