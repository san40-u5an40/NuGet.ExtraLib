...
**Статический класс `MessageBox`:**
 - `Show` — Метод вывода окна уведомлений. Принимает текст уведомления, заголовок окна и его тип (либо битовое поле `MessageBoxType`, либо объект `MessageBoxOptions`).

**Структура битового поля и объекта с опциями схожа:**
 - `WindowType` — Типы окон:
     - `Ok` — Только кнопка "Ок". (default)
 ...
 - `DefaultButton` — Выбранная кнопка по умолчанию:
     - `Button1` — Первая кнопка выбрана по умолчанию. (default)
     - `Button2` — Соответственно.
     - `Button3` — Соответственно.
     - `Button4` — Соответственно.
 - `IconType` — Иконка окна уведомления:
     - `None` — Отсутствие иконки. (default)
...
С объектом опций:
```C#
    MessageBoxOptions options = new(WindowType.OkCancel, IconType.Information, DefaultButton.Button2);
    MessageBox.Show("Это окно можно закрыть", "Информационное окно", options);
```
