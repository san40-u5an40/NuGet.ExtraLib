...
В данном NuGet-пакете представлена библиотека классов, расширяющая стандартные возможности программирования. В ней представлены паттерны, полезные как при процедурном стиле программирования, так и при объектно-ориентированном.
...
- `int.Random(int max)` — Случайное число от 0 до указанного значения.
- `int.Random(int min, int max)` — Случайное число в указанном диапазоне.
...
- `TryFormat` — Осуществляет попытку форматировать строку с указанными переменными, возвращая логическое значение об успешности, и саму форматированную строку при помощи out-параметра.
- `IsValidForInternalization` — Проверка валидности строки на предмет интернирования указанного количества переменных.
...
```C#
string str = "{0} {1}";
bool isSuccessFormat = str.TryFormat(out string formatted, 10, 12);
```

`IsValidForInternalization`:
```C#
string str = "{0} {1}";
...
- Конструктор, где и указывают начальные данные цепочки и токен завершения (в случае AsyncChain).
...
**Статические методы:**
 ...
 - `ThrowIfNotValid` — Кидает исключение `VerifierException`, если модель не валидна, хранящее сообщения об ошибках и саму модель.
...
### Примеры кода
Модель с указанными атрибутами:
```C#
public class Something
{
    public Something(string maskForIntern, string textWithContainsValue) =>
        (MaskForIntern, TextWithContainsValue) = (maskForIntern, textWithContainsValue);

    [InternalizationSupported(1)] // Поддержка интернирования одной переменной в строку
    public string MaskForIntern { get; private init; }

    [Contains("TEST")]            // Содержание подстроки "TEST"
...
Добавлены:
- Атрибуты для валидации моделей: `ContainsAttribute` и `InternalizationSupportedAttribute`.
- Метод `Broad.Patterns.Verifier.ThrowIfNotValid` и связанное с этим исключение `VerifierException`.
- Методы расширения для типа `Object`: `Broad.ObjectExtensions`.
- Методы `Broad.StringExtensions.TryFormat` и `Broad.StringExtensions.IsValidForInternalization`.
...
- Улучшен `Nuget.Health`.

### v3.0.0
- Добавлены тесты для ключевых классов библиотеки!
- Добавлена история последних изменений!
- В `Broad.Patterns` добавлен потокобезопасный объект `Readyable` (с сопутствующими ассоциированными типами), проверяемый на готовность и упрощающий задачи с отложенной инициализацией данных.
- В `Broad.Patterns.Chain` добавлена поддержка out-параметров, возвращающих промежуточные значения цепочки в виде объекта `Readyable`.
- Также появилась `Broad.Patterns.AsyncChain`, унаследовавшая все возможности обычной цепочки, но поддерживающая асинхронные операции. 
- Добавлена потокобезопасная интеграция `Broad.Patterns.Result` с `Readyable` подробнее в соответствующем [разделе](#result). 
...
- `Broad.Regexes` был переформирован:
    - `EmailReplace` перемещён в новый класс `EmailsParser`, где присутствуют также и новые возможности для работы с email-адресами.
